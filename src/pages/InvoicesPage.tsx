...
import { Plus, FileText, Calendar, Building2, User, Search, MoreVertical, Edit, Copy, Trash2, Download } from 'lucide-react';
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="w-40">
                                            <DropdownMenuItem onClick={(e: React.MouseEvent) => handleEdit(e, invoice)} className="cursor-pointer font-medium text-slate-600 focus:text-slate-900 focus:bg-slate-50">
                                                <Edit className="mr-2 h-4 w-4" />
                                                Edit
                                            </DropdownMenuItem>
                                            <DropdownMenuItem onClick={(e: React.MouseEvent) => handleDuplicate(e, invoice)} className="cursor-pointer font-medium text-slate-600 focus:text-slate-900 focus:bg-slate-50">
                    ...
                    <DialogContent className="w-full max-w-[95vw] md:max-w-3xl max-h-[90vh] overflow-y-auto p-0 gap-0 bg-white/95 backdrop-blur-xl rounded-xl sm:rounded-2xl">
                        <div className="p-4 sm:p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0">
                            ...
                            <div className="text-left sm:text-right w-full sm:w-auto">
                                ...
                                <p className="text-2xl sm:text-3xl font-black text-emerald-600">₹{viewingInvoice.totalAmount.toLocaleString()}</p>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                {viewingInvoice.customerName && (
                                    <div className="flex items-center gap-2 mb-2">
                                        <User className="h-4 w-4 text-slate-400" />
                                        <span className="font-bold text-slate-900 break-words">{viewingInvoice.customerName}</span>
                                    </div>
                                )}
                                {viewingInvoice.companyName && (
                                    <div className="flex items-center gap-2">
                                        <Building2 className="h-4 w-4 text-slate-400" />
                                        <span className="font-medium text-slate-600 break-words">{viewingInvoice.companyName}</span>
                                    </div>
                                )}
                            </div>
                            <div>
